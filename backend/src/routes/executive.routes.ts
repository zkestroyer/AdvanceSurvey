...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const newShopsToday = await prisma.shop.count({ where: { createdAt: { gte: today } } });
    const totalSurveys = await prisma.surveyResponse.count();
    const surveysToday = await prisma.surveyResponse.count({ where: { startedAt: { gte: today } } });
    const surveysThisWeek = await prisma.surveyResponse.count({ where: { startedAt: { gte: startOfWeek } } });
    const surveysThisMonth = await prisma.surveyResponse.count({ where: { startedAt: { gte: startOfMonth } } });
    ...
    const totalUsers = await prisma.user.count({ where: { role: { name: 'TSO' } } });
    
    const territoriesCovered = await prisma.territory.count({
      where: { shops: { some: {} } }
    });
    
    const citiesCovered = await prisma.city.count({
      where: { areas: { some: { name: { not: '' } } } } // A basic proxy, or could query shops distinct city
    });

    const products = await prisma.product.count();
    const brands = await prisma.productBrand.count();
    const categories = await prisma.productCategory.count();

    const completedSurveys = await prisma.surveyResponse.count({ where: { status: 'completed' } });
    const pendingSurveys = await prisma.surveyResponse.count({ where: { status: 'draft' } });
    ...
    res.json({
      ...
      data: {
        ...
        geographicCoverage: { territoriesCovered, citiesCovered, areasCovered: await prisma.area.count() },
        productInsights: { totalCategories: categories, totalBrands: brands, totalProducts: products, totalDistributors: await prisma.shop.count({ where: { type: 'Distributor' } }) },
...
router.get('/dashboard/charts', async (req, res) => {
  try {
    ...
    const surveyTrend: Record<string, number> = {};
    recentSurveys.forEach(s => {
      const date = s.startedAt.toISOString().split('T')[0] as string;
      surveyTrend[date] = (surveyTrend[date] || 0) + 1;
    });

    // Territory Performance
    const territoryData = await prisma.territory.findMany({
...
router.get('/users/:id', async (req, res) => {
  try {
    ...
    const surveys = await prisma.surveyResponse.count({ where: { userId: user.id } });
...
router.post('/comparison', async (req, res) => {
  try {
    ...
    const selectedEntities: string[] = filters?.entities || [];
    let tableData: any[] = [];
    let totalShops = 0;
    let avgPurchase = 0;
    ...
    if (groupBy === 'Brand') {
      const products = await prisma.product.findMany();
      let brands = [...new Set(products.map(p => p.brand).filter(Boolean))] as string[];
      ...
      for (const brand of brands) {
        ...
        const shopsWithBrand = await prisma.shop.count({
          where: {
            responses: {
              some: {
                answers: {
      ...
      let distributors = await prisma.shop.findMany({
      ...
      const products = await prisma.product.findMany();
      let categories = [...new Set(products.map(p => p.category).filter(Boolean))] as string[];
      ...
      const cityMap: Record<string, { shopCount: number }> = {};
      for (const s of shops) {
        const city = s.city || 'Unknown';
        if (selectedEntities.length > 0 && !selectedEntities.includes(city)) continue;
        if (!cityMap[city]) cityMap[city] = { shopCount: 0 };
    ...
    totalShops = tableData.reduce((sum: number, r: any) => sum + r.shopCount, 0);
    avgPurchase = tableData.length > 0 
      ? tableData.reduce((sum: number, r: any) => sum + r.purchasePrice, 0) / tableData.length 
      : 0;
    ...
    res.json({
      ...
      data: {
        ...
        table: tableData.sort((a: any, b: any) => b.shopCount - a.shopCount)
      }
    });
  } catch (error) {
    console.error(error);
