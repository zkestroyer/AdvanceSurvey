...
router.post('/templates', async (req, res) => {
    try {
        ...
        const template = await prisma_1.prisma.surveyTemplate.create({
...
router.get('/templates', async (req, res) => {
    try {
        const templates = await prisma_1.prisma.surveyTemplate.findMany({
            include: {
                sections: {
                    include: {
                        questions: true
...
router.get('/templates/:id', async (req, res) => {
    try {
        const template = await prisma_1.prisma.surveyTemplate.findUnique({
...
router.get('/active', async (req, res) => {
    try {
        const activeTemplate = await prisma_1.prisma.surveyTemplate.findFirst({
            where: { isActive: true },
...
router.post('/sections', async (req, res) => {
    try {
        ...
        const section = await prisma_1.prisma.surveySection.create({
                ...
                try {
                    ...
                    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed) && parsed.metadata) {
                        if (parsed.metadata.feedback) {
                            pseudoAnswers.push({ questionId: 'comments', value: parsed.metadata.feedback, question: { section: { title: 'Responses' } } });
                        }
                        if (parsed.metadata.checkoutLat) {
                            pseudoAnswers.push({ questionId: 'checkout_lat', value: String(parsed.metadata.checkoutLat), question: { section: { title: 'Responses' } } });
                        }
                        if (parsed.metadata.checkoutLng) {
                            pseudoAnswers.push({ questionId: 'checkout_lng', value: String(parsed.metadata.checkoutLng), question: { section: { title: 'Responses' } } });
                        }
                    }
                }
                catch (e) { }
...
router.post('/submit', async (req, res) => {
    ...
    try {
        ...
        const validQuestions = await prisma_1.prisma.surveyQuestion.findMany({
        ...
        const surveyResponse = await prisma_1.prisma.surveyResponse.create({
            data: {
...
router.get('/my-history', async (req, res) => {
    try {
        const userId = req.user?.userId || 1;
        const history = await prisma_1.prisma.surveyResponse.findMany({
            where: { userId },
            include: {
                shop: true,
                template: true,
...
router.put('/responses/:id', async (req, res) => {
    try {
        ...
        const responseRecord = await prisma_1.prisma.surveyResponse.findUnique({ where: { id: responseId } });
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.surveyAnswer.deleteMany({ where: { responseId } }),
            prisma_1.prisma.surveyAnswer.createMany({
                data: answers.map((a) => ({
                    responseId,
                    questionId: parseInt(a.questionId),
                    value: String(a.value),
...
router.post('/config', async (req, res) => {
    try {
        ...
        for (const el of schema) {
            if (el.type === 'section') {
                currentSection = await prisma_1.prisma.surveySection.create({
                    data: {
                ...
                await prisma_1.prisma.surveyQuestion.create({
                    data: {
...
router.put('/config/:id', async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.surveyTemplate.findUnique({ where: { id: templateId } });
        if (!existing)
            return res.status(404).json({ success: false, message: 'Survey not found' });
        // Update title
        await prisma_1.prisma.surveyTemplate.update({
            where: { id: templateId },
            data: { title }
        });
        // Delete existing sections (cascade deletes questions)
        await prisma_1.prisma.surveySection.deleteMany({
            where: { templateId }
        });
        let currentSection = null;
        let sectionOrderIndex = 0;
...
router.delete('/:id', async (req, res) => {
    try {
        const templateId = parseInt(req.params.id);
        await prisma_1.prisma.surveyTemplate.delete({
            where: { id: templateId }
        });
